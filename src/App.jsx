...
export default function App() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/tasks`);
  ...
  const deleteTask = async (id) => {
    await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/tasks/${id}`, { method: "DELETE" });
    setTasks(tasks.filter((task) => task._id !== id));
  };

  return (
    <div className="flex flex-col items-center min-h-screen bg-gradient-to-br from-purple-100 to-pink-100 p-4">
      <div className="bg-white shadow-2xl rounded-3xl p-8 w-full max-w-lg backdrop-blur-lg bg-opacity-90">
        <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 mb-6 text-center">
          Todo List
        </h1>
